...
const units = {
    ...
    alk:   '(umol/kg)',
    dic:   '(umol/kg)',
    Net_Capture:       '(tCO2/tCa(OH)2)',
    Net_Cost:          '($/tCO2)',
    Days_at_Sea:       '(days)',
    Emission_Maritime: '(tCO2/tCa(OH)2)',
    Cost_Maritime:     '($/tCa(OH)2)',
  };
...
function buildLayerButtons(variables) {
    ...
    Emission_Maritime: 'Shipping Emissions',
    Cost_Maritime:     'Shipping Cost',
  };
...
document.getElementById('runModel').addEventListener('click', async () => {
   ...
   const ss = parseFloat(document.getElementById('shipSpeed').value);
   const sz = parseFloat(document.getElementById('shipSize').value);
   const hf = parseFloat(document.getElementById('hfoPrice').value);
   const om = parseFloat(document.getElementById('omegaArag').value);
   const ds = parseFloat(document.getElementById('dissEff').value);
   const fc = parseFloat(document.getElementById('f_capacity').value);
   const country = document.getElementById('country').value;
   if (isNaN(gp) || isNaN(fp) || isNaN(ep) || isNaN(ei) || isNaN(ss) || isNaN(sz) || isNaN(hf) || isNaN(om) || isNaN(ds) || isNaN(fc) || !country) {
     outputEl.classList.remove('hidden');
     loaderEl.classList.add('hidden');
     resultEl.classList.remove('hidden');
     resultEl.innerHTML = '<span class="text-red-400">Please fill in all fields.</span>';
     return;
   }

   const params = {gasPrice: gp, fuelPrice: fp, elecPrice: ep, elecCIntensity: ei, 
                   shipSpeed: ss, shipSize: sz, hfoPrice: hf, omegaArag: om, dissEff: ds, 
                   f_capacity: fc, country };
